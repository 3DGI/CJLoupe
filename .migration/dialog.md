...
- `components.json` remains on legacy style `new-york`. There is no `base-new-york` registry counterpart, so changing it would silently retarget and restyle future additions. The shadcn CLI will continue to classify this config as Radix; future components should be migrated manually or added from an explicitly chosen Base style.
